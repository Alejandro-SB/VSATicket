...
namespace VSATicket.Application.Features.Users.LoginUser
{
    public class LoginUserHandler
    {
        ...
        {
            var result = await _userRepository.PasswordSignInAsync(command.UserName, command.Password);

            if (!result.Succeeded)
                return null;

            var user = await _userRepository.GetUserByNameAsync(command.UserName);
            if (user == null)
                return null;
            ...
            var claims = new[]
            {
                ...
                new Claim(ClaimTypes.Role, user.Role) // Použití vlastnosti Role
            };
